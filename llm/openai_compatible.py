...
什么应该保存：
- 名字、昵称、希望被怎样称呼
- 所在国家、城市、学校、专业、工作、实习、留学等稳定身份信息
- 稳定喜好，例如“最近迷上冰美式了”“火锅我能连吃三天”
- 稳定反感或禁忌，例如“香菜这东西我是真吃不了”“以后别叫我老师”
- 长期习惯、身份、关系状态、重要事实，例如“我在布里斯托留学”
...
def chat_once(
    ...
    if runtime_context:
        system_prompt = (
            f"{system_prompt}\n\n"
            "以下是本次回复的运行时上下文，它表示你自己的当前时间，不代表用户所在地时间。"
            "回答“你那边”“你那里”“你的时间”时优先参考这里。"
            "如果上下文里给出了根据记忆推断的用户当前时间，回答用户那边、用户说的“我这边”时优先使用它。"
            "回答英国/中国等地点时间时，要结合核心资料、长期记忆和聊天上下文判断：\n"
            f"{runtime_context}"
        )
